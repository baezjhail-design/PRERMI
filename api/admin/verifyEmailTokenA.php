<?php
// api/admin/verifyEmailTokenA.php - Verificar email de admin
...
if (!$token) {
    showVerificationPage(false, 'Token inválido o no proporcionado');
}

try {
    $pdo = getPDO();
    
    // Buscar admin con este token
    $stmt = $pdo->prepare("SELECT id, usuario, email FROM usuarios_admin WHERE verification_token = ?");
    $stmt->execute([$token]);
    $admin = $stmt->fetch();
    ...
    showVerificationPage(false, 'Error en la verificación. Intenta de nuevo.');
}
...
function showVerificationPage($success, $message) {
    ...
    <head>
        ...
        <title><?php echo $success ? 'Verificación Exitosa' : 'Error de Verificación'; ?> - PRERMI</title>
    ...
    <body>
        <div class="verification-card">
            ...
            <a href="../../web/admin/loginA.php" class="btn-back">Volver al Login</a>
        </div>
    </body>
    </html>
    <?php
